...
export default {
  ...
  computed: {
    tagGroup() {
      return [...this.achieveTypeText, ...this.resultFormText, ...this.processStatusText, ...this.ownerYearText]
    },
  },
  methods: {
    // 标签关闭
    closeTag(e) {
      switch (e.name) {
        case '成果类型':
          this.achieveType = this.achieveType.filter((item) => item != e.value)
          this.achieveTypeText = this.achieveTypeText.filter((item) => item.value != e.value)
          break
        case '成果形式':
          this.resultForm = this.resultForm.filter((item) => item != e.value)
          this.resultFormText = this.resultFormText.filter((item) => item.value != e.value)
          break
        case '成果状态':
    ...
    handleChange(checked, labelName, tagValue, tagLabel) {
      if (checked) {
        switch (labelName) {
          case '成果类型':
            this.achieveType = this.achieveType.filter((item) => item != '')
            this.achieveType.push(tagValue)
            this.achieveTypeText = this.achieveTypeText.filter((item) => item.value != '')
            this.achieveTypeText.push({ name: labelName, value: tagValue, label: tagLabel })
            break
          case '成果形式':
            this.resultForm = this.resultForm.filter((item) => item != '')
            this.resultForm.push(tagValue)
            this.resultFormText = this.resultFormText.filter((item) => item.value != '')
            this.resultFormText.push({ name: labelName, value: tagValue, label: tagLabel })
            break
          case '成果状态':
        ...
        switch (labelName) {
          case '成果类型':
            this.achieveType = this.achieveType.filter((item) => item !== tagValue)
            this.achieveTypeText = this.achieveTypeText.filter((item) => item.label !== tagLabel)
            break
          case '成果形式':
            this.resultForm = this.resultForm.filter((item) => item !== tagValue)
            this.resultFormText = this.resultFormText.filter((item) => item.label !== tagLabel)
            break
          case '成果状态':
    ...
    handleChecked(labelName, tagValue) {
      ...
      switch (labelName) {
        case '成果类型':
          changedGroup = this.achieveType
          break
        case '成果形式':
          changedGroup = this.resultForm
          break
        case '成果状态':
    ...
    handleSearch() {
      // 处理空值
      let achieveType = this.achieveType
      let resultForm = this.resultForm
      let processStatus = this.processStatus
      let ownerYear = this.ownerYear
      let keyWord = this.searchKey
      if (this.achieveType.indexOf('') != -1) {
        achieveType = []
      }
      if (this.resultForm.indexOf('') != -1) {
        resultForm = []
      }
      ...
      // 搜索条件
      let params = {
        achieveType: achieveType.toString(),
        resultForm: resultForm.toString(),
        processStatus: processStatus.toString(),
        ownerYear: ownerYear.toString(),
        searchKey: keyWord,
      }
