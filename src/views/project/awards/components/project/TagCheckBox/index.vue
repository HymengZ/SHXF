...
export default {
  ...
  computed: {
    tagGroup() {
      return [...this.awardTypeText, ...this.processStatusText, ...this.ownerYearText]
    },
  },
  methods: {
    // 标签关闭
    closeTag(e) {
      switch (e.name) {
        case '奖项类型':
          this.awardType = this.awardType.filter((item) => item != e.value)
          this.awardTypeText = this.awardTypeText.filter((item) => item.value != e.value)
          break
        case '奖项状态':
    ...
    handleChange(checked, labelName, tagValue, tagLabel) {
      if (checked) {
        switch (labelName) {
          case '奖项类型':
            this.awardType = this.awardType.filter((item) => item != '')
            this.awardType.push(tagValue)
            this.awardTypeText = this.awardTypeText.filter((item) => item.value != '')
            this.awardTypeText.push({ name: labelName, value: tagValue, label: tagLabel })
            break
          case '奖项状态':
        ...
        switch (labelName) {
          case '奖项类型':
            this.awardType = this.awardType.filter((item) => item !== tagValue)
            this.awardTypeText = this.awardTypeText.filter((item) => item.label !== tagLabel)
            break
          case '奖项状态':
