    ...
    <div class="container">
      <div class="search-tab">
        <a-tabs v-model="activeKey" @change="handleTabChange" >
          <a-tab-pane key="submitList" v-if="jurisdiction('achievements:submitShow') > -1">
            <div slot="tab">我提交的</div>
            <submit-list ref="submitList" />
          </a-tab-pane>
          <a-tab-pane key="applyList" v-if="jurisdiction('achievements:auditShow') > -1">
...
export default {
  ...
  data() {
    return {
      activeKey: this.jurisdiction('achievements:submitShow') > -1 ? 'submitList' : 'applyList',
    }
  },
  methods: {
    //遍历权限
