    ...
    <a-modal v-on="$listeners" :visible="visible" @cancel="handleCancel" @ok="hanldeOk" width="50%" title="审核">
      ...
      <a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 15 }">
        <a-form-item label="审核结果">
          <a-radio-group v-decorator="['operaCode', validatorRules.operaCode]" @change="handleOpinionRequired($event)">
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        operaCode: { initialValue: '', rules: [{ required: true, message: '审核结果不能为空' }] },
  ...
  methods: {
    ...
    handleOpinionRequired(e) {
      if (this.adoptValue == '200') {
        this.validatorRules.opinion.rules[0].required = e.target.value === '400' || e.target.value === '500'
      } else {
        this.validatorRules.opinion.rules[0].required = e.target.value === '140'
      }
    },
