...
export default {
  ...
  methods: {
    openPreview(record) {
      if (this.activeTab == 1) {
        if (this.jurisdiction('establishmentSubmit:details') > -1) {
          this.$router.push({
            name: 'project-establishment-preview-@id-@type',
            params: { id: record.id, type: 1 },
          })
        } else {
      ...
      if (this.activeTab == 2) {
        if (this.jurisdiction('establishmentApply:details') > -1) {
          this.$router.push({
            name: 'project-establishment-preview-@id-@type',
            params: { id: record.id, type: 2 },
          })
        } else {
          this.$message.warning('无预览权限')
        }
