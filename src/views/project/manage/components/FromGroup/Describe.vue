  ...
  data() {
    return {
      ...
      validatorRules: {
        ...
        expectAchieve: { rules: [{ required: true, message: '预期成果形式不能为空' }] },
        expectAchieveStr: {
          rules: [
            {
      ...
      url: {
        ...
        // 查询附件
        attachment: '/fire/projectReport/select/attachment/',
        // 数据字典接口
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          postAction(this.url.add, params).then((res) => {
            ...
            this.$emit('choiceDataType', this.describeData.id)
          })
        } else {
          // 预期成果形式校验
